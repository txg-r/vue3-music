...
import { addListenHistory, useDetail, useSongUrl, getOneMusicRecommend } from "@/utils/api";
...
export const usePlayerStore = defineStore({
    ...
    actions: {
        ...
        oneRecommand() {
            getOneMusicRecommend().then(res => {
                this.play(res.data.id)
            })
        }
        ,
        ...
        interval() {
            ...
            if (this.currentTime == 29 || this.currentTime == 89 || this.currentTime == 149) {
                addListenHistory(this.id)
            }
        }
    }
})
